#!/bin/sh
# PROD: odnowienie tylko certyfikatu rewers.edu.pl (wraz z www).
set -eu
cd -- "$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
case "${1:-}" in
    '') ;;
    --dry-run) ;;
    *) echo 'Uzycie: sh renew-cert-rewers.edu.pl-prod.sh [--dry-run]' >&2; exit 2 ;;
esac
[ "$#" -le 1 ] || exit 2
...
docker run --rm \
    ...
    certbot/certbot:v5.8.0 renew --cert-name rewers.edu.pl \
